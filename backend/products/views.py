...
class ProductListCreateAPIView(UserQuerySetMixin, StaffEditorPermissionMixin, generics.ListCreateAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        # On the view it's self.request, if it's on the serializer it's self.context.get('request') --> The view will definitely have the request, the serializer may or may not.
...
# Just another way of importing the view.
...
# A way of having everything in a single view with mixins.
...
# A way of doing the create/list views with function-based-views:
@api_view(["GET", "POST"])
